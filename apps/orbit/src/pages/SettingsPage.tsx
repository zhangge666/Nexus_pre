...
export default function SettingsPage(): React.JSX.Element {
  ...
  function renderContent(): React.JSX.Element {
    ...
    if (section === "cards") return <SettingsSectionLayout meta={SECTIONS.cards}>
      <SettingRow label="生成方式" id="generation-mode"><div className="radio-group"><label className={`radio-option${settings.cards.generationMode === "ai" ? " active" : ""}`}><input id="generation-mode" type="radio" name="generation-mode" checked={settings.cards.generationMode === "ai"} onChange={() => update("cards", { generationMode: "ai" })} />AI 自动生成</label><label className={`radio-option${settings.cards.generationMode === "manual" ? " active" : ""}`}><input type="radio" name="generation-mode" checked={settings.cards.generationMode === "manual"} onChange={() => update("cards", { generationMode: "manual" })} />手动创建</label></div></SettingRow>
    ...
    if (section === "review") return <SettingsSectionLayout meta={SECTIONS.review}>
      <SettingRow label="算法" id="algorithm"><div className="radio-group"><label className={`radio-option${settings.review.algorithm === "fsrs" ? " active" : ""}`}><input id="algorithm" type="radio" name="algorithm" checked={settings.review.algorithm === "fsrs"} onChange={() => update("review", { algorithm: "fsrs" })} />FSRS（推荐）</label><label className={`radio-option${settings.review.algorithm === "sm2" ? " active" : ""}`}><input type="radio" name="algorithm" checked={settings.review.algorithm === "sm2"} onChange={() => update("review", { algorithm: "sm2" })} />SM-2</label></div></SettingRow>
