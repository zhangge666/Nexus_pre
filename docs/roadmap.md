...
> 指数退避已完成。Rust 桌面/arm64 严格检查、Kotlin 全应用编译和三档视口回归均已通过；本地发布流水线
> 已完成三 ABI APK/AAB 构建、环境变量正式签名入口、预览签名、验签、版本元数据和 SHA-256 校验。M5 代码
> 范围已闭环，但当前没有在线 Android 设备，里程碑仍需手机/平板双设备、Doze/弱网、正式证书和 Play 内测
> 轨道验收，尚不能标记为全部完成。详细状态见 [m5-mobile.md](m5-mobile.md#7-android-当前交付状态2026-07-26)
> 和 [android-release.md](android-release.md)。
