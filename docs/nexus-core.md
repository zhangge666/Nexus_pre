...
- **信封**：轻量 `nexus-sync` crate 使用 XChaCha20-Poly1305 加密结构化操作；`nexus-core` 重新导出该 API，工作区标识和实体键均由
  BLAKE3 keyed hash 派生，中继看不到明文 ID。每条信封由设备 Ed25519 身份签名，中继只接受已登记且未撤销设备的有效签名。
- **恢复**：256 位同步根密钥可编码为带校验位的 24 词 BIP39 恢复短语；短语和设备私钥只进入系统安全区或用户主动展示流程。
- **配对**：二维码携带 256 位一次性秘密，会话中继只保存用该秘密封装的根密钥密文；六位确认码仅用于两端人工核对，不参与密钥派生。
- **移动副本**：Android 不链接 `nexus-core`/SQLite，而是在应用私有目录保存 Keystore 密钥加密的版本化副本和待上传信封；每次写入先持久化 oplog，再尝试上传和确认。

### 7.2 冲突与删除

- 结构化字段字段级合并；正文冲突保留双版本并标记，交由用户/Orbit 处理。
- 删除用 **tombstone** 传播，确保各端与中继都清除对应密文。
- 版本向量区分因果先后与并发修改；并发记录使用与输入顺序无关的稳定决胜规则收敛，墓碑优先防止旧内容复活，失败版本保留在冲突留痕中。
