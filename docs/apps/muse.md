...
- 平台：**Windows + macOS 一期交付**；移动端二期（移动端是随手记的理想场景，见 [architecture.md](../architecture.md) §5.2 平台矩阵）。
...
| 阶段 | 范围 | 说明 |
|------|------|------|
| **MVP** | 全局热键唤起浮层 + 文字速记 + 失焦/回车即存 + 入库（ingest） | 打通「唤起 → 速记 → 入库」核心闭环，Win/Mac；死磕冷启动速度 |
| **二期** | 语音输入（本地 `Transcriber`）+ 收件箱 + 就地速查 | 语音与「先接住」体验 |
| **三期** | 整理为 Quill 笔记 + 送 Orbit 归纳/复习 + 置顶速记板 | 家族内联联动 |
| **四期** | 云端 ASR 可切 + 剪贴板监听 + 自动标签 | 增强，落实隐私护栏 |
| **五期** | **移动端**（随手记的理想场景，小组件/分享面板/语音） | 见平台矩阵与后台限制对策 |
