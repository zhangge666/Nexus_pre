...
- 最终平台：**Windows + macOS + Android + iOS**，**移动端为硬需求**；当前先交付 Android，iOS 放到路线图最后的 M8 收口（见 [architecture.md](../architecture.md) §5.2 平台矩阵）；Linux 放缓
...
因此移动端仍是 Orbit 的硬需求，但开发顺序调整为 **Android 优先、iOS 最后收口**：M5 先验证并交付 Android Tauri 关键路径，M6–M7 不并行开发 iOS，M8 再复用已经稳定的移动共享层完成 iOS。
...
Orbit 是一个 Tauri 2.0 应用（`apps/orbit/`），面向桌面（Win/macOS）与移动端。前端 React + `@nexus/ui`，Rust 侧 `src-tauri` 装配命令与插件；移动端先在 M5 交付 Android，iOS 平台工程和专属适配延后到 M8。
...
> **Android 关键路径优先验证**（呼应 architecture.md §6 的首要风险）：Orbit 移动端不是「桌面的缩小版」，而是硬需求。M5 先在 Android 打通「连到记忆库、复习、问答、本地通知」主干，再补桌面独有的重功能（如大图谱）。平台差异（后台任务、本地通知）通过 `platform-mobile` 的 trait 隔离；iOS 不与 Android 并行开发，统一在 M8 复用稳定能力并处理平台差异。
...
| 难点 | 影响 | 对策 |
|------|------|------|
| **Tauri 移动端成熟度与后台** | 移动端是硬需求，但 Tauri 移动端年轻，后台任务/本地通知/长连接受系统限制 | 平台适配层（`platform-mobile`）隔离差异；M5 **优先验证 Android 关键路径**（连库/复习/问答/通知），M8 再验证 iOS；到期提醒用系统本地通知调度而非常驻进程；必要时局部改用原生视图（architecture.md §2.1 的退路） |
...
对齐 architecture.md §5.2 平台矩阵与 roadmap 的节奏。移动端仍是 Orbit 的硬需求，但 M5 只交付 Android，iOS 放到 M8 最后收口；M3 只接入 Muse 最小来源，不并行建设完整兄弟产品。

| 阶段 | 目标 | 交付内容 |
|------|------|---------|
| **M1–M2 桌面独立交付** | 让中枢先独立可用 | 统一检索、集合、记忆编辑、时间线、桌面工作台、本地服务持有者与单实例仲裁；不要求安装 Echo/Muse/Quill |
| **M3 单来源验证** | 证明 Orbit 能承接独立应用 | 接收 Muse 最小文字来源，完成授权、事件刷新、检索/时间线展示、来源追踪、撤销与重连；不开发完整 Muse，也不开发 Echo/Quill |
| **M4 智能化** | 让第二大脑形成学习闭环 | 知识卡片生成、FSRS 间隔复习、带引用的 RAG 问答、Completion Provider 与关联发现 |
| **M5 Android 多端** | 让 Orbit 先在 Android 随身且可同步 | Android 关键路径、本地通知、跨设备 E2E 同步、多设备配对与恢复；不开发 iOS |
| **M6 全面外联** | 开放 Orbit 生态 | MCP Server、浏览器扩展剪藏、SDK/CLI，以及「连接与隐私」控制台的 scope、令牌、审计与导出 |
| **M7 产品族协作** | 承接完整兄弟产品 | Echo、Muse、Quill 产品化后，Orbit 只补充跨源消费与治理体验，不在 Orbit 内复制它们的捕获或编辑功能 |
| **M8 iOS 收口** | 最后补齐 Orbit iOS | 复用 Android 已稳定的移动共享层，完成 WKWebView、Keychain、本地通知、真机验收、签名与发布准备 |
