...
对齐 architecture.md §5.2 平台矩阵与 roadmap 的节奏。移动端仍是 Orbit 的硬需求，但按全局路线图在 M5 交付；M3 只接入 Muse 最小来源，不并行建设完整兄弟产品。
...
| **M5 多端** | 让 Orbit 随身且可同步 | iOS/Android 关键路径、本地通知、跨设备 E2E 同步、多设备配对与恢复 |
