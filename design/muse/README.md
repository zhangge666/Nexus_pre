...
- **静**：参考用户选定的低对比深色工作台效果，采用扁平时间线、弱分隔线和按需出现的右侧检查器，不使用大卡片、渐变背景或装饰性数据。
- **一致**：聚合工作流、灵感、任务、会议、剪贴板、设置和四个快捷工具窗使用同一套标题栏、列表、按钮、输入与焦点规则；常规圆角不超过 `8px`，不以阴影制造层级。
- **可调**：设置页提供石墨、暖砂、雾白、暮蓝与自定义配色；所有窗口继续消费同一组设计 token，主题修改即时同步且本地持久化。
...
正式应用已经拆成一个主窗口和四个独立工具窗，并完成全界面视觉统一：

| 区域 | Tauri 窗口 / React 入口 | 当前首要能力 |
|---|---|---|
| 聚合与设置 | `main` / `src/App.tsx` | 三栏工作流、扁平时间线、任务检查器、历史管理与可选 Orbit 连接 |
...
界面实现继续复用 `@nexus/ui` token；主窗口各页采用单层工作区、扁平行列表和按需检查器，快捷工具窗只保留当前动作所需控件。原型中的业务文案与数据仅用于展示，不代表尚未接入的录音、转写或后台剪贴板监听能力已经完成。
